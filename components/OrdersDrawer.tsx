        ...
        <div className="w-screen max-w-md bg-[#18181b] shadow-2xl flex flex-col">
          <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
            <div className="flex items-start justify-between">
              ...
              <button onClick={onClose} className="p-2 text-slate-400 hover:text-white transition-transform duration-300 hover:scale-105">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="mt-8 space-y-6">
              {orders.length === 0 ? (
                <div className="text-center py-12">
                  <div className="bg-[#0f1113] w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 border border-white/5">
                     <ClipboardList className="w-10 h-10 text-slate-500" />
                  </div>
                  <p className="text-slate-500 font-medium">No tenés pedidos recientes.</p>
                </div>
              ) : (
                orders.map((order) => (
                  <div key={order.id} className="bg-[#0f1113] rounded-2xl p-5 border border-white/5 relative overflow-hidden group shadow-sm">
                    ...
                    <div className="mb-3">
                      <p className="text-xs font-bold text-slate-500 uppercase">Orden #{order.id.slice(-6)}</p>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-white/10">
                      <span className="text-lg font-black text-white">${order.total.toLocaleString('es-AR')}</span>
                      
                      {/* Botón Inicial de Cancelar */}
                      {order.status === 'Pendiente' && confirmingId !== order.id && (
                        <button
                          onClick={() => handleCancelClick(order.id)}
                          className="text-xs font-black text-red-400 hover:text-red-300 bg-red-900/20 hover:bg-red-900/40 px-3 py-2 rounded-lg transition-all duration-300 hover:scale-105 flex items-center gap-1 border border-red-500/20"
                        >
                      ...
                      {confirmingId === order.id && (
                        <div className="flex items-center gap-2">
                           <button
                            ...
                            className="text-[10px] font-black bg-red-600 text-white px-3 py-2 rounded-lg hover:bg-red-700 transition-all duration-300 hover:scale-105 flex items-center gap-1 disabled:opacity-50 disabled:hover:scale-100"
                          >
                          ...
                          <button
                            ...
                            disabled={isCancelling}
                            className="text-[10px] font-black bg-white/10 text-slate-300 px-3 py-2 rounded-lg hover:bg-white/20 transition-all duration-300 hover:scale-105 flex items-center gap-1 disabled:opacity-50 disabled:hover:scale-100"
                          >
